...
import { PrimaryButton, GlassCard }  from '../components/componentsUtilityUI';// Import UI yang baru dibuat
...
const LoginPage = ({ handleLogin, isLoading }) => {
    ...
    const getPasswordBorderColor = () => {
        if (!isPasswordFocused || password.length === 0) return 'border-white/50';
        return password.length >= 8 ? 'border-green-500' : 'border-red-500';
    };

    const getPasswordRingColor = () => {
        if (!isPasswordFocused || password.length === 0) return 'focus:ring-red-500';
        ...
        <div className="flex items-center justify-center min-h-screen bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50 p-4 relative overflow-hidden">
            ...
            <GlassCard className="w-full max-w-md p-8 md:p-10 text-center relative z-10 bg-white/30 backdrop-blur-xl border border-white/30 shadow-2xl rounded-3xl">
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <PrimaryButton type="submit" className="w-full py-3 rounded-2xl bg-gradient-to-r from-blue-500 to-indigo-600 hover:from-blue-600 hover:to-indigo-700 shadow-lg transform transition-all duration-200 hover:scale-[1.02]" disabled={isLoading}>
                        {isLoading ? (
                            <>
